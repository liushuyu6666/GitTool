...
import { joinPathAndCreate } from '../../../utils/file/file';
...
export class GitIdxFile implements GitIdxFileInterface {
  ...
  constructor(filePath: string, outObjectDir: string) {
    ...
    // Make sure packPath already exists.
    const packPath = joinPathAndCreate(outObjectDir, 'pack');
    const outFile = path.join(packPath, `${fileName}.json`);
