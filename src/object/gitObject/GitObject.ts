...
import { GitOfsDeltaObjectData } from './GitOfsDeltaObjectData';
    ...
    if (isOriginalObject(type)) {
      ...
      switch (type) {
        case GitObjectType.OFS_DELTA:
          if (!offsets || !swapOffsets) {
            throw new Error(
              `[differentiation error]: need offsets and swapOffsets to differentiate OFS_DELTA object.`,
            );
          }
          this.data = new GitOfsDeltaObjectData(
            body,
            this.hash,
            offsets,
            swapOffsets,
